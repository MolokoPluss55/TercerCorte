...
function preload()
{
  ...
  table = loadTable("assets/vehiculosBogota.csv", "header");
}

//LOS DATOS SE GUARDAN EN LAS VARIABLE EN EL SETUP
...
function setup()
{
  ...
  for (let i = 0; i < rowCount; i++)
  {
    ...
    //se determina las posX y posY para ubicar posteriormente el texto del comcel sobre el eje x del diagrama
    //se replican las lineas 14 veces, en las pocisiones dentro del espacio del diagrama
    let x = map(i, 0, rowCount-1, 55, 475); //desde la 0 hasta la 13
   
    //se mapean los datos de comcel acorde a la altura del diagrama
    let yVehiculo = map(vehiculos, 0, maxDato, 300, 100);

    //Adiciono al arreglo un objeto de tipo circulo, donde obtiene los datos de comcel para dibujarlos en forma de circulo
  //  diagramaCirculos.push(new circulo (x,yVehiculo));

    //Adiciono al arreglo un objeto de tipo circulo, donde obtiene los datos de comcel para dibujarlos en forma de circulo
    diagramaBarras.push(new barra (x,yVehiculo));
...
function draw()
{
  ...
  //13121911/8 = 1640238
  //2000000
  for (let i = 0; i < maxDato; i+=200000)
  {
